...
void printIdentification(struct VL6180xIdentification *temp){
	  ...
	  pc.printf("Model Rev = ");
  	pc.printf("%d",temp->idModelRevMajor);
  	pc.printf(".");
  	pc.printf("%d\r\n",temp->idModelRevMinor);

  	pc.printf("Module Rev = ");
  	pc.printf("%d",temp->idModuleRevMajor);
  	pc.printf(".");
  	pc.printf("%d\r\n",temp->idModuleRevMinor);  

  	pc.printf("Manufacture Date = ");
  	pc.printf("%d",((temp->idDate >> 3) & 0x001F));
  	pc.printf("/");
  	pc.printf("%d",((temp->idDate >> 8) & 0x000F));
  	pc.printf("/1");
  	pc.printf("%d\r\n",((temp->idDate >> 12) & 0x000F));
  	pc.printf(" Phase: ");
  	pc.printf("%d\r\n",(temp->idDate & 0x0007));

  	pc.printf("Manufacture Time (s)= ");
  	pc.printf("%d\r\n",(temp->idTime * 2));
  	pc.printf("\r\n\r\n");
}

void released(){
	if(debounceTimer.read_ms() > 50){
		but = 1;
	}
}
...
void Button(){
	...
	myButton.fall(&released);
}

void action0(){
	pc.printf("X0=%d     X1=%d     X2=%d     X3=%d     X4=%d     X5=%d     X6=%d\r\n",x0,x1,x2,x3,x4,x5,x6);
	Button();
	pc.printf("Wait to Start\r\n\r\n");
	if (but == 1){
		pc.printf("Start\r\n\r\n");
		but = 0;
		x0 = 0;
...
void action2(){
	...
	if(i != 0){
		...
		pc.printf("SUCCESS TO INITIALIZE\r\n\r\n");  
 	    x2 = 0;
 	    x3 = 1;
  }
	wait_ms(500);	
...
void action4(){ 
		pc.printf("X0=%d     X1=%d     X2=%d     X3=%d     X4=%d     X5=%d     X6=%d\r\n",x0,x1,x2,x3,x4,x5,x6);
    pc.printf("Ambient Light Level (Lux) = ");   //Get Ambient Light level and report in LUX
    pc.printf("%f\r\n\r\n\r\n",sensor.getAmbientLight(GAIN_1) );
  	x4 = 0;
  	x5 = 1;
  	wait_ms(500); // delay 1s
}

void action5(){
		pc.printf("X0=%d     X1=%d     X2=%d     X3=%d     X4=%d     X5=%d     X6=%d\r\n",x0,x1,x2,x3,x4,x5,x6);
		pc.printf("Distance measured (mm) = ");      //Get Distance and report in mm
    pc.printf("%d\r\n\r\n\r\n", sensor.getDistance() ); 
    x5 = 0;
  	x6 = 1;
  	wait_ms(500); // delay 1s
...
int main() {
	x0 = 1; x1 = 0; x2 = 0; x3 = 0; x4 = 0; x5 = 0; x6 = 0; but = 0;
	grafcet();  
}
